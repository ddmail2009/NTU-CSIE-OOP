...
public class LWC_NPC_Mary extends LWC_NPC {
    
    public LWC_NPC_Mary(String name, String group, LWC_RPG rpg) {
    ...
    public LWC_AbstractInterface getMsg(){
        if(!msg.load_conversation(name, rpg.getPlotState())){
            ...
            if(getCondition().equals("Self")){
                rpg.getPlayer().addItem(new ItemLoader("config/Item.txt").getItem(rpg,"Pie"));
            }
        }
        else
            setCondition(rpg.getPlotState());
